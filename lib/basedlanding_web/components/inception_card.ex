defmodule BasedLandingWeb.Components.InceptionCard do
  ...
  attr :scale, :float, default: 0.95  # Made scale closer to 1 for less dramatic scaling
  attr :rotation, :integer, default: 3  # Reduced rotation for clarity
  ...
  def inception_card(assigns) do
    ~H"""
    <div class="relative">
      <div class={"absolute top-0 left-0 w-full transform transition-all duration-500"}
           style={"transform: scale(#{:math.pow(@scale, @depth)}) rotate(#{@rotation * @depth}deg); top: #{@depth * 10}px; left: #{@depth * 10}px"}>
